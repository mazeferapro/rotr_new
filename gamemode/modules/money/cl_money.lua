...
local money_color = Color(255, 215, 0, 255) -- Золотой цвет по умолчанию
...
function NextRP.MoneyHUD:UpdateMoneyColor()
    ...
    elseif time_since_update < 5 then
        ...
        money_color = Color(255, 215 + (40 * fade), 0, 255)
    else
        -- Обычный золотой цвет
        money_color = Color(255, 215, 0, 255)
    end
end
...
-- Хук для периодического обновления
hook.Add("Think", "NextRP_MoneyHUD_Update", function()
    -- Проверяем каждые 0.1 секунды
    if (CurTime() % 0.1) < FrameTime() then
        GetCurrentMoney()
    end
end)
