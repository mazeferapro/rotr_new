...
local function Kitsune_ArrestMenu(pl, Cal)
    ...
    for _, target in pairs(pl) do
        if Cal:GetPos():Distance(target:GetPos()) < 1000 then
                ...
                :On('DoClick', function()
                    LIB:DoStringRequest('Время', 'Введите время в минутах', '0', function(v)
                        --[[net.Start('KitsuneArest')
                            ...
                            net.WriteString(v)
                        net.SendToServer()]]--
                        netstream.Start('NextRP::ArrestArresting', target, v) print(v)
...
netstream.Hook('NextRP::OpenArrestMenu', function(Cal, pl)
    Kitsune_ArrestMenu(pl, Cal)
end)
