    ...
    MySQLite.query(
        ...
        function(tPlayerData)
            if tPlayerData and istable(tPlayerData) then
                ...
                pPlayer:AddIGSFunds(100, 'Бонус за ввод реферального кода.')
                pPlayer:SavePlayerData( 'referal_code_applied', sCode )
                pPlayer:SetNVar('nrp_referalcodeapplied', sCode)

                pPlayer:SendMessage(MESSAGE_TYPE_SUCCESS, 'Вы ввели реферальный код! Начислено 100 рублей!')

                local targetPlayer = player.GetBySteamID64(tPlayerData[1].community_id)

                if IsValid(targetPlayer) then
                    targetPlayer:AddIGSFunds(100, 'Бонус за ввод реферального кода другом.')
                    targetPlayer:SendMessage(MESSAGE_TYPE_WARNING, 'Ваш реферальный код ввели! Начислено 100 рублей!')
                else
                    IGS.Transaction(tPlayerData[1].community_id, 100, 'Бонус за ввод реферального кода другом.')
                end
            else
