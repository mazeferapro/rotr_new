...
function NextRP.Cadets:OpenUI()
    ...
    local ustav = vgui.Create('DButton', self.content)
        ...
        ustav:TDLib()
            ...
            :On('DoClick', function()
                gui.OpenURL('https://sites.google.com/view/the-charter-of-var/главная?authuser=0')
            end)
    ...
    local discord = vgui.Create('DButton', self.content)
        ...
        discord:TDLib()
            ...
            :On('DoClick', function()
                gui.OpenURL('https://discord.gg/buaJZ2SJm3э')
            end)
    ...
    if number > 10 then
        ...
        if st then
            local title = vgui.Create('PawsUI.Panel', self.content)
                :Stick(TOP, 1)
                :Text('Тест сдан! Перейдите в дискорд 882-го!', 'PawsUI.Text.Large', NextRP.Style.Theme.Green)
            ...
            ctdisc:TDLib()
                ...
                :FadeHover(NextRP.Style.Theme.Blue)
                :CircleClick(NextRP.Style.Theme.AlphaWhite)
                :Text('Дискорд 882-го', 'PawsUI.Text.Normal', NextRP.Style.Theme.Text)
                :On('DoClick', function()
                    gui.OpenURL('https://discord.gg/pWFtjPckcp')
                end)
