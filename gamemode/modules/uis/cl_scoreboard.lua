...
function scoreboard:ScoreboardToggle(bToggle)
    if bToggle then
        ...
        local Header = TDLib('DPanel', stuffBlock)
            ...
            :ClearPaint()
            :On('Paint', function(s, w, h)
                draw.SimpleText(NextRP.Config.FancyTitle, 'font_sans_26', 64 + 15, h * .5 - 9, NextRP.Style.Theme.HightlightText, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
                draw.SimpleText("Онлайн: "..#player.GetHumans()..'/'..game.MaxPlayers(), 'font_sans_16', 64 + 15, h * .5 + 13, NextRP.Style.Theme.Text, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
